...
import { BackIcon, Button, MoreIcon, Popper } from '~/components'
import { LANGUAGE, NON_USER_ITEMS, USER_ITEMS } from '~/constants'
        ...
        return (
          <Popper.MenuItem key={index} className={cx('wrapper-item', { separate: isSeparate })}>
            <Button
              ...
              LeftIcon={<Icon />}
              onClick={
                isSeparate ? handleSignOut : isLanguages ? handleChangeShowLanguages.bind(null, true) : undefined
              }
            >
              {title}
            </Button>
